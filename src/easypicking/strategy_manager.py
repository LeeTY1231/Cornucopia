...
class StrategyManager:
    ...
    def auto_discover_strategies(self) -> None:
        ...
        try:
            # 获取目录名作为模块名
            dir_name = os.path.basename(os.path.abspath(self.strategy_dir))
            # 导入目录作为包
            strategies_module = importlib.import_module(dir_name)
            # 扫描目录下的所有Python文件
            for _, module_name, is_pkg in pkgutil.iter_modules(
                strategies_module.__path__, 
                strategies_module.__name__ + "."
            ):
                if is_pkg:
                    continue
                try:
            ...
            if (inspect.isclass(obj) and 
                ...
                try:
                    ...
                    if strategy_name in self.strategy_classes:
                        self.logger.warning(f"策略名称冲突: {strategy_name}，将使用 {obj.__name__}")
    ...
    def get_strategy(self, strategy_name: str, 
                    create_new: bool = True) -> Optional[StrategyTemplate]:
        """
        获取策略实例
    ...
    def execute_strategy(self, strategy_name: str, 
        ...
        if not strategy.validate_parameters(execution_params):
            missing_params = [p for p in strategy.required_params 
                            if p not in execution_params]
        ...
        try:
            ...
            self.logger.info(f"策略 {strategy_name} 执行成功，选中 {len(result.selected_stocks)} 只股票")
            return result
        except Exception as e:
    ...
    def batch_execute(self, strategies: List[Dict[str, Any]], 
                     data: pd.DataFrame) -> Dict[str, StrategyResult]:
        """
        批量执行策略
